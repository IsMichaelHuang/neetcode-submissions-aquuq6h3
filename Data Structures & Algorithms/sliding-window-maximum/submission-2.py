    ...
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:  
        ...
        for i in range(k):
            if not mono_stack:
                ...
                # Add the biggest value to the left
                if nums[i] > nums[mono_stack[0]]:
        ...
        for r in range(k, len(nums)):
            # If greatest value is the leaving left bound
            ...
            if not mono_stack:
                ...
                if nums[r] > nums[mono_stack[0]]:
                    mono_stack.appendleft(r)
                else:
                    # deque purge
